...
//Insere um no folha
void insereNoFolha(TNoFolha *no, int cod, TFunc *p, int d, FILE *arq_Dados, TMetadados *metadados, int pos,
    ...
    if(no->m < 2*d) {//Caso caiba
        ...
        int j = 0;
        while (j < no->m && cod > no->funcs[j]->cod) {
            novo->funcs[j] = no->funcs[j];
            j++;
        }
        ...
        TFunc *guardaFunc[(2 * d) + 1];
        int j = 0;
        //Guarda todos os clientes em um vetor
        while (j < no->m && cod > no->funcs[j]->cod) {
            guardaFunc[j] = no->funcs[j];
            j++;
        }
        ...
        for (k1 = 0; k1 < no->m + 1; k1++) {
            if (k1 < meio) {
                novo->funcs[k1] = guardaFunc[k1];
                novo->m++;
            } else if (k1 >= meio) {
                novo2->funcs[k2] = guardaFunc[k1];
                novo2->m++;
                k2++;
            }
        }
        ...
        if (no->pont_pai != -1) { //Caso tenha um pai
            ...
            novoPai->chaves[0] = guardaFunc[meio]->cod;
            novoPai->m = 1;
...
int busca_binaria(void *r,int *pos, int chave, int esq, int dir, int folha){
    ...
    *pos = meio;
    if(folha){
        if(dir >= esq){
            TNoFolha *raiz = r;
            if(raiz->funcs[meio]->cod < chave){
                return busca_binaria(raiz, pos, chave,meio + 1, dir, folha);
            }
            else if(raiz->funcs[meio]->cod == chave){
                *pos = meio;
                return chave;
            }
...
int insere(int cod, char *nome, float preco, char *nome_arquivo_metadados, char *nome_arquivo_indice,char *nome_arquivo_dados, int d)
{
...
//Remove um funcionario do no folha
TNoFolha *retira_pizza(TNoFolha *no, int pos){
    int i;
    for(i = pos; i < no->m - 1; i++){
        if(no->funcs[i+1]){
            no->funcs[i] = no->funcs[i+1];
        }
    }
    no->funcs[no->m-1] = NULL;
    no->m--;
    return no;
}
...
void redistribui_dir(TNoFolha *dir, TNoFolha *esq, TNoInterno *pai){
    esq->funcs[esq->m] = dir->funcs[0];//passei do irmao da direita pro da esquerda;
    int cod = esq->funcs[esq->m]->cod;
    ...
    pai->chaves[pos] = dir->funcs[0]->cod;//atualizei o indice
    esq->m++;
}
...
void redistribui_esq(TNoFolha *dir, TNoFolha *esq, TNoInterno *pai){
    int cod = esq->funcs[esq->m-1]->cod;
    dir = shift(dir);
    dir->funcs[0] = esq->funcs[esq->m-1];
    int pos = 0;
...
void concatena(TNoFolha *no, TNoFolha *irmao, TNoInterno *pai, int d, int op, int ind_pai ,int *index, FILE *dados){
    if(op == DIR) {
        ...
        for (int i =0; i < no->m; i++) {
            ...
            irmao->funcs[irmao->m] = no->funcs[i];
            irmao->m++;
        }
...
void carrega_dados(int d, char *nome_arquivo_entrada, char *nome_arquivo_metadados, char *nome_arquivo_indice, char *nome_arquivo_dados) {
    ...
    funcio = le_funcionarios(nome_arquivo_entrada);

    for (int i = 0; i < funcio->qtd; i++) {

        insere(funcio->lista[i]->cod,
               funcio->lista[i]->nome,
               funcio->lista[i]->preco,
...
int buscaFuncionario(int cod, char *nome_arquivo_metadados, char *nome_arquivo_indice, char *nome_arquivo_dados, int d){
    ...
    if(buscaFolha(pos, nome_arquivo_dados, cod, d)) {
        ...
        TNoFolha *noFolha = le_no_folha(d, arq_Dados);
        int i;
        for(i = 0; cod != noFolha->funcs[i]->cod; i++);
        TFunc *p = noFolha->funcs[i];
        imprime_func(p);
        fclose(arq_Dados);
        return 1;
    }else return 0;
...
void tarefa(int opt, char *nome_arquivo_metadados, char *nome_arquivo_indice, char *nome_arquivo_dados, int d) {
	...
	int max_funcionario=30;
	int numFuncionarios = 100;
	int count = 0;
	int cod=0;
	while (count < numFuncionarios) {
			...
			float salario = (float)rand() / RAND_MAX * 10000;
        ...
        if(!imprimirArvore(d, nome_arquivo_dados,nome_arquivo_indice, nome_arquivo_metadados)) printf("Sem funcionario cadastrado.\n");
